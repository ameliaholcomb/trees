...
def run_sample(sample_num, capture_num, width):
    ...
    print("New mode: {}".format(mode_depth))
    ...
    # #####
    # trunk_pts = img_rgbd[:,left:right,:]
    # flat_trunk_depths = trunk_pts[:,:,3][trunk_pts[:,:,3] != 0].flatten()
    # print(left)
    # print(right)
    # print(img_rgbd.shape)
    # print(trunk_pts.shape)
    # print(np.max(flat_trunk_depths))
    # idx = np.argpartition(flat_trunk_depths, 5)
    # print(flat_trunk_depths[idx[:5]])
    # print(set(flat_trunk_depths).sorted())
    ...
    mask = np.logical_or(np.logical_or(depth_matrix == 0, depth_matrix == 0.0), depth_matrix > 5.0)
    ...
    # show depth key at appropriate size
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.05)
    fig.colorbar(cs, cax)
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
    ax.set_title("RGB Image with Depth Overlaid")

    with sns.axes_style("whitegrid"):
        ax = fig.add_subplot(223)
    depth_matrix_center_values = np.ndarray.flatten(center_depths[center_depths != 0])
    sns.histplot(data=depth_matrix_center_values, bins=bins)
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.05)
    cax.set_visible(False)
    ax.set_xlabel("Depth (m)")
    ax.set_ylabel("Number of pixels")
    ax.set_title("Depth Values in Center Third of Image")
    ...
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
    ax.set_title("RGB Pixels Filtered by Estimated Trunk Depth")


    # plt.subplots_adjust(wspace=0.3)
    plt.tight_layout()

    plt.savefig(os.path.join(RESULT_PATH, "Example.pdf"))
    plt.close()
